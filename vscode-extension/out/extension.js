...
function activate(context) {
    ...
    context.subscriptions.push(autoIntegrateCmd, showTraceViewerCmd, generateManimVideoCmd, exportDiagramCmd);
    // Watch for trace file changes
    setupTraceWatcher(context);
}
async function autoIntegrateProject(context) {
    ...
    vscode.window.showInformationMessage(`TrueFlow integrated! Run with: TRUEFLOW_ENABLED=1 python ${selected.label}`);
    ...
    const action = await vscode.window.showInformationMessage('Would you like to create a VS Code launch configuration?', 'Yes', 'No');
    if (action === 'Yes') {
        // TODO: Add to .vscode/launch.json
        vscode.window.showInformationMessage('Launch configuration created!');
    }
}
function showTraceViewer(context) {
    if (traceViewerPanel) {
        traceViewerPanel.reveal(vscode.ViewColumn.Two);
        return;
    }
    traceViewerPanel = vscode.window.createWebviewPanel('trueflowTraceViewer', 'TrueFlow Trace Viewer', vscode.ViewColumn.Two, {
        enableScripts: true,
...
async function generateManimVideo() {
    ...
    const traceDir = config.get('traceDirectory', './traces');
    const pythonPath = config.get('pythonPath', 'python');
    const quality = config.get('manimQuality', 'medium_quality');
...
function setupTraceWatcher(context) {
    const config = vscode.workspace.getConfiguration('trueflow');
    const traceDir = config.get('traceDirectory', './traces');
    const watcher = vscode.workspace.createFileSystemWatcher(new vscode.RelativePattern(vscode.workspace.workspaceFolders[0], `${traceDir}/**/*.json`));
